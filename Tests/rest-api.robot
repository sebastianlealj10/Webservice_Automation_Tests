...
System must make a delete request
    ...
    [Tags]  Webservices
    create session  localhost  http://localhost:300
    &{data}=      Create Dictionary   id=${idnumber}  title=newjsonserver  author=sebas
    &{header}=    Create Dictionary  content-type=application/json
    ${response} =  delete request  localhost  /posts/  json=${data}  headers=${header}
    should be equal as strings  ${response.status_code}  200
    ${json} =  set variable  ${response.json()}
    log  ${json}
    should be equal as strings  ${response.json()}  {u'author': u'sebas', u'id': u'${idnumber}', u'title': u'newjsonserver'}